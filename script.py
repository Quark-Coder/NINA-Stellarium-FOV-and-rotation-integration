...
def send_stellarium_command(command):
    ...
    try:
        ...
        if response.status_code != 200:
            print( Fore.RED + f"POST Error: {response.status_code}, Response: {response.text}" + Fore.RESET)
    except requests.exceptions.RequestException as e:
        print(Fore.RED + f"POST Request failed for command '{command}': {e}" + Fore.RESET)

def get_nina_angle():
    try:
        response = requests.get(NINA_ADVANCED_API_URL)
        response.raise_for_status()
        data = response.json()
        return data.get("Response", {}).get("Position", "Unknown")
    except (requests.exceptions.ConnectionError, json.JSONDecodeError) as e:
        print(Fore.RED + f"Error getting NINA angle: {e}" + Fore.RESET)
        return None
...
if __name__ == "__main__":
    try:
        ...
        print(Fore.BLUE + "This script allows you to transfer FOV and camera rotation directly to Stellarium in real time." + Fore.RESET)

        config = load_config()

        telescopeControlSlot = config.get("telescopeControlSlot") or input(Fore.CYAN + "[Telescope Control] Enter "
                                                                                       "telescope slot: "+ Fore.RESET)
        ocularsCameraIndex = config.get("ocularsCameraIndex") or input(Fore.CYAN +"[Oculars] Enter camera "
                                                                                  "index: "+ Fore.RESET)
        ocularsTelescopeIndex = config.get("ocularsTelescopeIndex") or input(Fore.CYAN +"[Oculars] Enter telescope"
                                                                                        " index: "+ Fore.RESET)
        ...
        while running:
            try:
                angle = get_nina_angle()
                if angle is not None:
                    send_stellarium_command(f"Oculars.setSelectedCCDRotationAngle({angle})")
            except Exception as angle_error:
                print(Fore.RED + "Error obtaining NINA angle:" + Fore.RESET, angle_error)
            time.sleep(0.2)
